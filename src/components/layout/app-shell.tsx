...
export function AppShell({ children }: { children: React.ReactNode }) {
  ...
  const { contracts, source, updatedAt } = useWorkspace();
  ...
  return (
    <div className="min-h-dvh">
      ...
      <aside className="fixed inset-y-0 start-0 z-30 hidden w-64 border-e bg-background/88 px-4 py-5 backdrop-blur-xl lg:flex lg:flex-col">
        ...
        <div className="mt-4 font-mono text-[10px] text-muted-foreground">MODEL · STRESS ENGINE v1.0</div>
      </aside>
      ...
      <header className="sticky top-0 z-20 flex h-16 items-center justify-between border-b bg-background/88 px-4 backdrop-blur-xl lg:ms-64 lg:px-8">
        ...
        <div className="flex items-center gap-3">
          <div className="hidden text-end sm:block">
            ...
            <p className="text-[10px] text-muted-foreground">계약 {contracts.length.toLocaleString("ko-KR")}건 · {new Date(updatedAt).toLocaleTimeString("ko-KR", { hour: "2-digit", minute: "2-digit" })} 갱신</p>
          </div>
